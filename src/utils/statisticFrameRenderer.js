...
/**
 * Measures the width of a string with per-character letter spacing,
 * matching the Python version's approach:
...
/**
 * Loads an image from a src URL and returns a promise.
 */
function loadImage(src) {
  return new Promise((resolve, reject) => {
    const image = new Image()
    image.onload = () => resolve(image)
    image.onerror = () => reject(new Error('Failed to load image. The file may be corrupted or unsupported.'))
...
/**
 * Renders the statistic frame on a canvas.
 * Port of statistic_frame.py
 *
 * Python logic reference:
 *   1. Open image, create RGBA overlay
 *   2. Uppercase text, split by \n
 *   3. Measure each line with textbbox
 *   4. Position from bottom: y = img.height - totalHeight - 50
 *   5. For each line: draw white rounded rect bg (alpha 180/255),
 *      then draw each char with white fill + black stroke (width=5)
 *   6. Alpha composite overlay onto image
 */
export async function renderStatisticFrame(canvas, imageSrc, topText, bottomText) {
